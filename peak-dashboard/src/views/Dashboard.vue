<template>
    <div class="dashboard p-4 h-screen">
        ...
        <!-- end nav -->
        <div id="divAgentList" class="text-center absolute z-10 rounded-md box-border shadow-xl" v-show="showAgentList">
            <div id="divAgentListheader" class="rounded-tl-md rounded-tr-md text-white cursor-move flex"
        ...
        <div class="h-full" v-if="groupMode">
            <v-chart :option="optionGroup" autoresize :loading="loading" :loadingOptions="loadingOptions"
                @click="handleGroupClick" />
        </div>
        <div class="h-full" v-if="!groupMode">
            <v-chart :option="optionMembers" autoresize :loadingOptions="loadingOptions" />
        </div>
    </div>
</template>
